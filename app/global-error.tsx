...
export default function GlobalError({
  ...
  return (
    <html lang="id">
      <body className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 sm:p-6 selection:bg-indigo-500 selection:text-white font-sans antialiased">
        <div className="w-full max-w-lg rounded-3xl border border-slate-800 bg-slate-900/90 backdrop-blur-xl p-6 sm:p-8 shadow-2xl space-y-6 text-center">
          ...
          <div className="space-y-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/20 text-[11px] font-bold text-rose-400">
              ...
              Sistem Recovery Aktif
            </span>
            <h2 className="text-xl sm:text-2xl font-black tracking-tight text-white">
              Terjadi Kesalahan Tak Terduga
            </h2>
            <p className="text-xs sm:text-sm text-slate-400 max-w-sm mx-auto leading-relaxed">
              {error?.message || 'Aplikasi mendeteksi gangguan sementara. Tenang, data sesi lokal Anda tetap tersimpan dengan aman.'}
            </p>
          </div>
